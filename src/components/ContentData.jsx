          ...
          : content &&
            content.map((item, i) => (
              <Link key={i} to={`/${type}/detail/${item.id}`} className={`max-w-sm mx-auto shadow-none border-0 transition-shadow mb-4 duration-300 ${isVisible ? "visible" : ""}`}>
                <img
                  src={item.poster_path ? `${process.env.REACT_APP_BASEIMGURL}/${item.poster_path}` : "/placeholder-img.png"}
                  alt={item.title || item.name || "No Image Preview"}
                  className="w-full sm:w-64 md:w-72 lg:w-80 h-auto object-fit rounded-lg"
                />
                ...
                <div className="p-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <p className="flex font-normal text-gray-700 dark:text-gray-400">
                      <img src={StarIcon} alt="Star Rating Icon" style={{ width: "17px", height: "22px", marginRight: "5px" }} /> <span className="Rate">{Math.round(item.vote_average * 10) / 10}</span>
                    </p>
                  </div>
                </div>
              </Link>
