...
import { getMovieList, nowPlaying, searchMovie, topRated, upcoming, airingToday } from "./api";
...
function App() {
  ...
  return (
    <div className="relative container mx-auto dark:bg-black py-24">
        ...
        <Navbar.Collapse className="">
          ...
          <div className="absoulte left-0 mt-1">
            <Dropdown label={<span className="text-xl dark:text-white"> TV Shows </span>} inline>
              <Dropdown.Item className="text-lg pr-20" onClick={() => categoryFilter("airing_today")}>
                {" "}
                Airing Today{" "}
              </Dropdown.Item>
      ...
      <h1 className="text-2xl font-semibold dark:text-white"> High Popularity </h1> <ItemList />
    </div>
  );
}
